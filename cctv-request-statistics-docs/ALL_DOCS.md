...
| 31 | Master data ทั้ง 4 ชุดลากเรียงลำดับได้โดยไม่กรอกตัวเลข | ใช่ |
| 32 | วางแล้วบันทึกลำดับอัตโนมัติเป็น `1..N` โดยไม่ซ้ำ | ใช่ |
| 33 | บันทึกลำดับล้มเหลวแล้ว UI คืนตำแหน่งเดิม | ใช่ |
...
- [x] จัดการประเภทผู้ขอ
- [x] จัดการสถานะ
- [x] เพิ่ม/แก้ไข/เปิดปิดใช้งาน โดยรายการใหม่ต่อท้ายอัตโนมัติ
- [x] ลากเรียงลำดับพร้อม auto-save, validation และ rollback เมื่อบันทึกล้มเหลว
...
- [x] เพิ่มรายการใหม่
- [x] แก้ไขชื่อ
- [x] ลากเรียงลำดับและบันทึกอัตโนมัติหลังวาง โดยระบบกำหนด `1..N` เอง
- [x] เปิด/ปิดใช้งาน
...
- [x] เอาช่องกรอก `sort_order` ออกและให้รายการใหม่ต่อท้ายจากลำดับสูงสุด
- [x] ตรวจ ID ครบ/ซ้ำ/เกินก่อน reorder และคืนตำแหน่งเดิมเมื่อบันทึกล้มเหลว
- [x] แก้ create ชื่อซ้ำให้ปฏิเสธแทนการแก้รายการเดิมแบบเงียบ ๆ
- [x] ตรวจและซ่อม `sort_order` เดิมที่ซ้ำหรือมีช่องว่างระหว่างเตรียม schema
...
งาน performance ล่าสุดแยกข้อมูลช่วยเหลือที่ไม่จำเป็นออกจากการแสดงฟอร์มหลัก และเพิ่ม loading skeleton สำหรับหน้าเพิ่มคำร้อง/รายงาน เพื่อให้ระบบตอบสนองทันทีแม้ runtime บน Vercel หรือ Neon จะมีจังหวะ cold start
...
| จัดการ Master Data | เพิ่ม แก้ไข เปิด/ปิด และลากเรียงประเภทผู้ขอ หมวดหมู่ สถานะ และประเภทหลักฐาน |
...
- จัดการประเภทหลักฐาน
- เปิด/ปิดใช้งาน
- drag-and-drop sort order ที่บันทึกอัตโนมัติหลังวาง

Verification:
- รายการ inactive ไม่แสดงในฟอร์มใหม่
- รายงานย้อนหลังยังแสดงชื่อเดิม
- ไม่ hard delete รายการที่ถูกใช้แล้ว
- ไม่มีช่องกรอกเลขลำดับ และรายการใหม่ต่อท้ายอัตโนมัติ
- หลังลาก `sort_order` เป็น `1..N` โดยไม่มีเลขซ้ำหรือช่องว่าง
- เมื่อบันทึกล้มเหลว UI คืนลำดับเดิม
...
ใช้รูปแบบเดียวกันกับประเภทผู้ขอ หมวดหมู่ สถานะ และประเภทหลักฐาน
...
```text
เปิดหน้าจัดการประเภทผู้ขอ หมวดหมู่ สถานะ หรือประเภทหลักฐาน
-> เพิ่มหรือแก้ไขชื่อ
-> รายการใหม่ถูกต่อท้ายอัตโนมัติ
-> จับไอคอนลากรายการไปยังตำแหน่งใหม่
-> เมื่อวาง ระบบบันทึกลำดับทั้งหมดทันที
-> เปิดหรือปิดใช้งาน
...
- ผู้ใช้ไม่ต้องกรอกเลขลำดับเอง
- หลังบันทึก ลำดับต้องเป็นเลขต่อเนื่อง `1..N` และแต่ละรายการต้องมีตำแหน่งเดียว
- การบันทึกลำดับต้องส่งรายการทั้งหมดในชุดเดียว เพื่อตรวจ ID ซ้ำ ขาด หรือเกินก่อนอัปเดต
- ถ้าบันทึกหลังวางไม่สำเร็จ UI ต้องคืนลำดับเดิมและแจ้งข้อผิดพลาด
- การปิดหรือเปิดใช้งานไม่เปลี่ยนตำแหน่งของรายการโดยอัตโนมัติ
